...
resource "aws_subnet" "AWX-dummy-subnet1" {
  vpc_id            = aws_vpc.vpc-tf.id
  availability_zone = var.availability_zone1
  cidr_block        = "100.127.1.0/24"

  tags = {
    Name = "AWX-dummy-subnet1"
  }
}
...
# Create the IGW
resource "aws_internet_gateway" "igw-AWX-dummy" {
  vpc_id = aws_vpc.vpc-tf.id
...
resource "aws_route" "default_to_igw" {
  ...
  gateway_id             = aws_internet_gateway.igw-AWX-dummy.id

}
##  Notice that this will upload my public key to AWS and use it for the EC2s.  
##  This way, I can ssh-in with my private keys.
##  so, first do:   cp ~/.ssh/id_rsa.pub   ./.certs

resource "aws_key_pair" "loginkey1" {
  key_name = try("AWX-login-key") #  using function try here.  If key is already present don't mess with it
...
#  network_interface_id = aws_instance.dummy-AWX-ec2[count.index].primary_network_interface_id
#}

resource "aws_instance" "dummy-AWX-ec2" {
  ami                         = data.aws_ami.std_ami.id
  instance_type               = var.instance_type
  subnet_id                   = aws_subnet.AWX-dummy-subnet1.id
...
resource "null_resource" "update" {
  depends_on = [aws_instance.dummy-AWX-ec2]
  triggers = {
...
resource "null_resource" "awx" {
  ...
  provisioner "remote-exec" {
    ...
    connection {
      ...
      host        = aws_instance.dummy-AWX-ec2[count.index].public_ip
    }
  }
}
...
output "publicIP" {
  ...
  value = {
    for instance in aws_instance.dummy-AWX-ec2 :
